...
uint8_t uart_it_1_callback_string_byte; //extern data
...
void uart_it_1_callback_parser(UART_HandleTypeDef *UartHandle)
{
	...
	buffer[i] = uart_it_1_callback_string_byte;
	i++;
	if(i >= it_1_string_out_buffer_length) {
		memset((char *)string1_out, '\0', it_1_string_out_buffer_length);
...
uint8_t uart_it_2_callback_string_byte; //extern data
...
void uart_it_2_callback_parser(UART_HandleTypeDef *UartHandle)
{
	...
	buffer[i] = uart_it_2_callback_string_byte;
	i++;
	if(uart_it_2_callback_string_byte == 0x0A  &&  buffer[i-2] == 0x0D) {
		memset((char *)string2_out, '\0', it_2_string_out_buffer_length);
...
uint8_t uart_it_3_callback_string_byte; //extern data

static uint8_t string3_out[4][it_3_string_out_buffer_length];
...
void uart_it_3_callback_parser(UART_HandleTypeDef *UartHandle)
{
	...
	buffer[i] = uart_it_3_callback_string_byte;
	i++;
	if(uart_it_3_callback_string_byte == 0x0A  &&  buffer[i-2] == 0x0D) {
		...
		switch(count) {
			...
			case 1:
				buffer3_flag2 = 0;
				memset((char *)*(string3_out+1), '\0', it_3_string_out_buffer_length);
			...
			case 2:
				buffer3_flag3 = 0;
				memset((char *)*(string3_out+2), '\0', it_3_string_out_buffer_length);
			...
			case 3:
				buffer3_flag4 = 0;
				memset((char *)*(string3_out+3), '\0', it_3_string_out_buffer_length);
